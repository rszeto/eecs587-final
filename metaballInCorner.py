...
def main(argv):
    ...
    for loopVar in range(numImages):
        ...
        for i in range(numBalls):
            row = np.random.randint(0, pointRange[0])
            col = np.random.randint(0, pointRange[1])
            balls.append(Ball((row,col), size=r))
        # Draw the balls
        simpleDrawBalls(image, balls, goo, threshold, renderRange)
        fileName = 'metaballsInCorner%02d.png' % loopVar
...
def simpleDrawBalls(image, balls, goo, threshold, renderRange):
    imSize = image.shape
    assert(imSize[0] >= renderRange[0] and imSize[1] >= renderRange[1])
    for row in range(renderRange[0]):
        for col in range(renderRange[1]):
            sum = 0
            for i in range(len(balls)):
                ball = balls[i]
